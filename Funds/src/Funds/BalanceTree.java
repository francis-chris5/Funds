...
import Funds.Dialogs.AccountLedgerDialog;
...
public class BalanceTree extends StackPane{
    ...
    private AccountLedgerDialog accountDialog;
    ...
    public void launchAccountDialog(){
        ...
        try{
            ...
            accountDialog = new AccountLedgerDialog(book, acc);
    ...
    public class BalanceTreeContextMenu extends ContextMenu{
        ...
        public BalanceTreeContextMenu(){
            ...
            miNewAccountCategory.setOnAction(m ->{
                ...
                AccountCategoryDetailsDialog temp = new AccountCategoryDetailsDialog(book, findType(tree.getRoot().getValue().toString()));
                //book.setSaved(false);
                book.displayDetails();
            });
            ...
            miNewAccountDirect.setOnAction(m -> {
                //AccountType localType = findType(tree.getRoot().getValue().toString());
                AccountDetailsDialog temp = new AccountDetailsDialog(book, findType(tree.getRoot().getValue().toString()));
                //book.setSaved(false);
                book.displayDetails();
            });
            ...
            if(!subcategory.isEmpty()){
                for(int i = 0; i < subcategory.size(); i++){
                    ...
                    item.setOnAction(m -> {
                        //AccountType localType = findType(tree.getRoot().getValue().toString());
                        AccountDetailsDialog temp = new AccountDetailsDialog(book, findType(tree.getRoot().getValue().toString()), category);
                        //book.setSaved(false);
                        book.displayDetails();
                    });
